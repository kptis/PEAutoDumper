...
def call_scan(data_vr_address, data, start_limit=None, end_limit=None):
    ...
    #
    start_limit = data_vr_address if start_limit is None else start_limit
...
def rebuild_iat(pid, pe_data, base_address, oep, newimpdir="MTAimpdir", newiat="MTAiat", loadfrommem=True):
    ...
    """
    logger.info("Starting IAT rebuild for PID %d" % pid)
    ...
    print "OEP: 0x%x" %oep
    print "Image Base: 0x%x " %base_address
    print "RVA OEP: 0x%x" %rva_oep
    logger.debug("Base address: 0x%x, OEP: 0x%x, RVA_OEP: 0x%x" % (base_address, oep, rva_oep))
    ...
    for tmp_sec in pf.SHList:
        ...
        if (rva_oep >= tmp_start) and (rva_oep <= tmp_end):
            try:
                ...
                error_message =  "Errors due to wrong version of elfesteem, don't use pip, instead install from https://github.com/serpilliere/elfesteem"
    ...
    if pdata is None:
        error_message = "Unable to locate rva_oep: 0x%x in sections: %s" % (rva_oep, pf.SHList)
    ...
    if not iat_ptrs:
        error_message = "Unable to find IAT pointer candidates in code!"
        logger.error(error_message)
        raise AssertionError(error_message)
    ...
    if not iat_map:
        error_message = "The iat_map is empty, we are unable to find the new IAT pointers for the old pointers in imp_table."
        logger.error(error_message)
        raise AssertionError(error_message)
...
def dump_and_rebuild(pid, oep, newimpdir="MTAimpdir", newiat="MTAiat"):
    ...
    @param newiat: name for new section that will contain new IAT
    '''
    logger.info("Start dumping the original payload")
    System.request_debug_privileges()
    process = Process( pid )
    try:
        process.suspend()
